...
reconcile() {
  ...
  done < <(find "$root" -name '.listing' 2>/dev/null)
...
PYEOF
  ...
  CORE_VERIFIED=1
  local n_mod=0 n_add=0 n_miss=0 kind rel
  ...
  rep ""
  rep "Resumen core: alterados=$n_mod  ajenos=$n_add  ausentes=$n_miss"
  return 0
}
...
scan_tree() {
  ...
  done < <(find "$root" -path '*/uploads/*' \
           \( -name '*.php' -o -name '*.phtml' -o -name '*.php[0-9]' -o -name '*.phar' \) \
           2>/dev/null || true)

  # NOTA — emit_paths se alimenta con PROCESS SUBSTITUTION, nunca con un pipe.
  # Un pipe `grep | emit_paths` corre emit_paths en un subshell y pierde los
  # ++ de CRIT/REVIEW y los SCREEN_HITS al volver acá (el informe sí persiste
  # porque es un archivo). `< <(...)` mantiene emit_paths en este shell.

  # #4 — .htaccess con ejecución inyectada.
  rep ""; rep "── #4 .htaccess con PHP inyectado ──"
  emit_paths CRITICO ".htaccess: auto_prepend/handler PHP" ".htaccess inyectado" \
    < <(grep -rlE 'auto_prepend_file|auto_append_file|AddType[[:space:]]+application/x-httpd-php' \
        "$root" --include='.htaccess' 2>/dev/null)

  # #5 — Firmas de webshell conocidas.
  rep ""; rep "── #5 Firmas de webshell ──"
  emit_paths CRITICO "firma de webshell" "webshell" \
    < <(grep -rlE 'FilesMan|WSO [0-9]|c99shell|r57shell|b374k|IndoXploit|MiniShell' \
        "$root" --include='*.php' 2>/dev/null)
  ...
  emit_paths CRITICO "superglobal como función" "\$_[..]()" \
  ...
  # shellcheck disable=SC2016  # regex de grep, no expansión de shell
  emit_paths CRITICO "eval/preg_replace ofuscado" "eval(...)" \
    < <(grep -rlE 'eval[[:space:]]*\([[:space:]]*(base64_decode|gzinflate|str_rot13)|preg_replace[[:space:]]*\([[:space:]]*.[^,]*/e[^,]*.[[:space:]]*,' \
        "$root" --include='*.php' 2>/dev/null)
...
close_out() {
  ...
  info "Se conservan: informes de escaneo, reconciliación, fallo y cierre."
  ...
  info "3. Descargar los Raw Access Logs del cPanel — se rotan"
  info "4. Rotar credenciales: cPanel, FTP, base de datos, Cloudflare, pasarela"
  info "5. Erradicación sobre la copia, nunca sobre el origen"
  printf '\n'
